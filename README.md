...
| 方案    | 权重占用   | 建议显存 / 内存 | 示例 prefill | 成本 / 百万输入 token | 成本 / 1,000 次请求 | 估算 prefill 延迟 | 相对 Jev |
| ------- | ---------- | --------------- | ------------ | --------------------- | ------------------- | ----------------- | -------- |
| 2B      | 1.2-1.7 GB | 3-4 GB          | 2,500 tok/s  | ¥0.027                | ¥0.025              | 0.36 s            | 0.17×    |
| 4B      | 2.4-3.0 GB | 6-8 GB          | 1,800 tok/s  | ¥0.038                | ¥0.034              | 0.50 s            | 0.23×    |
| 9B      | 5.5-6.5 GB | 10-12 GB        | 1,000 tok/s  | ¥0.068                | ¥0.061              | 0.90 s            | 0.41×    |
| 28B     | 16-18 GB   | 20-24 GB        | 400 tok/s    | ¥0.170                | ¥0.153              | 2.25 s            | 1.02×    |
| 30B-A3B | 17-19 GB   | 20-24 GB        | 700 tok/s    | ¥0.097                | ¥0.088              | 1.29 s            | 0.59×    |
| Jev API | -          | -               | -            | ¥0.30                 | ¥0.15               | 70-500 ms         | 1.00×    |

`30B-A3B` 是总参数约 30B、每 token 激活约 3B 的 MoE 模型，显存仍须容纳全部专家，吞吐通常高于同量级 dense 模型。本地行的成本只计边际电费，延迟按 `900 input tokens / prefill` 估算，不含排队、decode、网络和硬件折旧。Jev 行使用 API 价格和[官方公布延迟范围](https://typesafe.ai/blog/introducing-system-one-models-and-jev)，延迟没有 SLA 或百分位数据。`相对 Jev` 以 Jev `¥0.15/千次` 为 1.00。
